...
svm.rs.log <- "result/extreme/svm.extreme.classification.ft.v3.log"
...
for(filter in acc.filters) {
  ...
  while(TRUE) {
    ...
    for(i in xft.try.ind) {
      ...
      svm.classifier <- get.three.classifier.svm(data.train, data.test, ft.ind, c.list, sigma.list, "rbf", "C-svc")
      if(filter == "train"){
    ...
    if(length(diff.ind) == 1){
      ...
      line.write <- sprintf("features = %s, train.acc = %f, test.acc = %f, train.high.acc = %f, train.medium.acc = %f, train.low.acc = %f, test.high.acc = %f, test.medium.acc = %f, test.low.acc = %f", 
                             svm.classifier.max$features, svm.classifier.max$train.acc, svm.classifier.max$test.acc, svm.classifier.max$train.high.acc, svm.classifier.max$train.medium.acc, svm.classifier.max$train.low.acc,
                             svm.classifier.max$test.high.acc, svm.classifier.max$test.medium.acc, svm.classifier.max$test.low.acc)
      print(line.write)
